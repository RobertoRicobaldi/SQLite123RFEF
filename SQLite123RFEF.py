...
if pagina == "Búsqueda de Jugadoras":
    ...
    if "LIGA" in df.columns:
        ligas = df["LIGA"].dropna().unique()
        liga_seleccionada = st.sidebar.selectbox("Selecciona una liga", ["Todas"] + list(ligas))
    else:
    ...
    if liga_seleccionada != "Todas" and "LIGA" in df.columns:
        df_filtrado = df_filtrado[df_filtrado["LIGA"] == liga_seleccionada]
    if equipo_seleccionado != "Todos" and "EQUIPO" in df.columns:
        df_filtrado = df_filtrado[df_filtrado["EQUIPO"] == equipo_seleccionado]

    jugadoras = df_filtrado["NOMBRE"].dropna().unique()
    seleccionada = st.selectbox("Selecciona una jugadora", ["Selecciona una jugadora"] + list(jugadoras), key="busqueda_jugadora")
...
if pagina == "Comparativa de Jugadoras":
    ...
    if jugadora_1 and jugadora_2:
        jugadora_1_data = df_filtrado[df_filtrado["NOMBRE"] == jugadora_1]
        jugadora_2_data = df_filtrado[df_filtrado["NOMBRE"] == jugadora_2]
        ...
        for jugadora in [jugadora_1, jugadora_2]:
            st.write(f"### Valoraciones de {jugadora}")
