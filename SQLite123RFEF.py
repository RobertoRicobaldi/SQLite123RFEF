...
if pagina == "Búsqueda de Jugadoras":
    ...
    if seleccionada:
        ...
        valores = datos[metricas].sum().tolist()

        fig = go.Figure()
        fig.add_trace(go.Scatterpolar(r=valores, theta=metricas, fill='toself', name=seleccionada))
        fig.update_layout(polar=dict(radialaxis=dict(visible=True)), showlegend=False)
        st.plotly_chart(fig)

        st.write("### Gráfico de Barras")
        fig_bar = px.bar(x=metricas, y=valores, labels={"x": "Métrica", "y": "Valor"}, title=f"Estadísticas de {seleccionada}")
